...
#actor id, actor name, film title, description, and release year.
   ...
   SELECT film_actor.actor_id AS actor_id, CONCAT (actor.first_name , ' ' ,  actor.last_name) as actor_name ,  film.title AS film_title , film.description AS film_descreption , film.release_year 
    FROM film
...
SELECT film.film_id , film.title , CONCAT_WS (' ' , first_name , last_name ) As actor_name , actor.actor_id 
from actor 
...
SELECT film.film_id,film.title , film.description , film.release_year , film.rating , film.special_features , category.name as category_name , film.rental_rate from film 
JOIN film_category ON film.film_id = film_category.film_id 
join category ON film_category.category_id = category.category_id 
